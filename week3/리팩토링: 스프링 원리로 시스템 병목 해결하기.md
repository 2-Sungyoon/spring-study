...
- **개념:** 기술을 몰라도 기능을 쓸 수 있게 해주는 것.
- **비유:** 운전자는 자동차 엔진이 가솔린인지 전기 모터인지 몰라도 엑셀 페달(인터페이스)만 밟으면 차가 나간다.
- **역할:** 컨트롤러가 구체적인 기술(Servlet, Cookie, JDBC)에 종속되지 않고, 추상화된 인터페이스(@LoginUser, Repository)만 바라보게 하여 기술이 바뀌어도 코드를 수정하지 않게 한다.

### 2. Bulkhead Pattern (격벽 패턴)
- **개념:** 시스템의 리소스를 물리적으로 격리하여 장애 전파를 막는 패턴.
- **비유:** 내부를 여러 칸(격벽)으로 나눠서 한 칸에 꽉 차도 다른 칸은 멀쩡하다.
- **역할:** AI 요약 기능의 스레드가 다 고갈되어도, 로그인이나 결제 같은 메인 기능의 스레드 풀은 **별도로 분리되어(격리)** 있으므로 시스템 전체가 멈추는 것을 방지한다.

### 3. Circuit Breaker
- **개념:** 외부 서비스에 장애가 생겼을 때, 계속 기다리지 않고 즉시 연결을 끊어버리는 안전장치.
- **비유:** 집안의 '두꺼비집'과 같은 역할. 과전류가 흐르면 불이 나기 전에 전기를 딱! 끊어서 보호한다.
- **역할:** 외부 API(AI 서버)가 응답하지 않을 때, 30초씩 기다리지 않고 즉시 에러(Fail-fast)를 반환하거나 기본값을 줘서 내 서버의 스레드가 무한 대기하는 것을 막는다.

### 4. N+1 문제와 Batch Fetching
- **개념:** 1번의 쿼리로 N개의 데이터를 가져왔는데, 그 N개의 데이터마다 연관된 정보를 얻기 위해 N번의 추가 쿼리가 나가는 성능 문제. (1 + N)
...
- **[결정]:**
  ...
  - <img width="699" height="490" alt="image" src="https://github.com/user-attachments/assets/fe2fade3-8099-4356-985f-3780c9b620a8" />
